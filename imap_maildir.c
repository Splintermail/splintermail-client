...
static derr_t ctn_check(const string_builder_t *path, bool *ret){
    ...
    *ret = true;
    for(size_t i = 0; i < 3; i++){
        ...
        bool temp;
        PROP( dir_rw_access_path(&subdir_path, false, &temp) );
        *ret &= temp;
    }
    return E_OK;
}
...
static derr_t find_children(const string_builder_t *base, const dstr_t *name,
    ...
    // open the maildir
    imaildir_t *m;
    PROP_GO( imaildir_new(&m, &parent->path, name), fail_malloc);
    // put this child into the parent's hashmap
    PROP_GO( hashmap_puts(&parent->children, &m->name, &m->h), fail_open);
