...
static derr_t list_of_tests_1(LIST(dstr_t)* ans, dstr_t* recv, dstr_t* buffer,
    ...
    // listhook
    {
        ...
        PROP( connection_write(conn, &temp) );
        DSTR_VAR(exp_1, 64);
        PROP( FMT(&exp_1, "+OK %x messages (%x octets)\r\n",
                          FU(ans->len), FU(len_sum)) );
        DSTR_VAR(expect, 256);
        for(size_t i = 0; i < ans->len; i++){
    ...
    for(size_t i = 0; i < ans->len; i++){
        size_t index = i + 1;
        DSTR_VAR(temp, 32);
        PROP( FMT(&temp, "RETR %x\r\n", FU(index)) );
        PROP( connection_write(conn, &temp) );
        DSTR_VAR(exp_1, 32);
        PROP( FMT(&exp_1, "+OK %x octets\r\n", FU(ans->data[i].len)) );
        DSTR_VAR(tag, 32);
        PROP( FMT(&tag, "retr %x", FU(index)) );
...
static derr_t test_ditm(void){
    ...
    PROP_GO( fake_pop_server_new(&fps, files, nfiles, uids), cu_ditm);
    // start fake pop_server and wait until its ready
    PROP_GO( fake_pop_server_start(&fps), cu_ditm);

    // load up the files that we should expect to receive from DITM
    LIST_VAR(dstr_t, ans, sizeof(files) / sizeof(*files));
    for(size_t i = 0; i < nfiles; i++){
        dstr_t temp;
        PROP_GO( dstr_new(&temp, 256), cu_ans);
        DSTR_VAR(path, 4096);
        PROP_GO( FMT(&path, "%x/%x", FS(g_test_files), FS(ansfiles[i])), cu_ans);
        error = dstr_read_file(path.data, &temp);
        if(!error) error = LIST_APPEND(dstr_t, &ans, temp);
        if(error){
            dstr_free(&temp);
            PROP_GO(error, cu_ans);
        }
    }

    // set up a couple of dynamically allocated buffers
    dstr_t recv;
    PROP_GO( dstr_new(&recv, 4096), cu_fps);
    dstr_t buffer;
    PROP_GO( dstr_new(&buffer, 4096), cu_recv);

    // prepare ssl context
    ssl_context_t ctx;
    PROP_GO( ssl_context_new_client(&ctx), cu_buffer);

    // now connect to the server
    ditm_thread_start_test();
    fps_start_test();
    connection_t conn;
    PROP_GO( connection_new_ssl(&conn, &ctx, "127.0.0.1", 1996), cu_ctx);
    LOG_INFO("test client connected\n");

    // run the list of tests for the first connection
    PROP_GO( list_of_tests_1(&ans, &recv, &buffer, &conn), cu_conn);
    PROP_GO( ditm_thread_end_test(), cu_conn);
    PROP_GO( fps_end_test(), cu_conn);
    ...
    PROP_GO( list_of_tests_2(&ans, &recv, &conn), cu_conn);
    PROP_GO( ditm_thread_end_test(), cu_conn);
    PROP_GO( fps_end_test(), cu_conn);
...
cu_fps:
    ...
    if(fps_error & E_ANY){
        LOG_CATCH(fps_error);
        LOG_ERROR("fake pop server exited with error %x\n",
                  FD(error_to_dstr(fps_error)));
...
static derr_t do_test_mangle(const char* infile, const char* outfile,
    ...
    if(infd < 0){
        LOG_ERROR("%x: %x\n", FS(infile), FE(&errno));
        ORIG(E_OS, "unable to open file for test");
    }
    int outfd = open(outfile, O_RDONLY);
    if(outfd < 0){
        LOG_ERROR("%x: %x\n", FS(outfile), FE(&errno));
        ORIG_GO(E_OS, "unable to open file for test", cu1);
    }
    // open a temp file
    int temp = open("do_test_mangle.temp", O_CREAT | O_RDWR | O_TRUNC, 0660);
    if(temp < 0)
        ORIG_GO(E_OS, "unable to create file for test", cu2);
    // allocate memory for the "answer"
    dstr_t answer;
    PROP_GO( dstr_new(&answer, 4096), cu3);
    // allocate memory for the test result
    dstr_t result;
    PROP_GO( dstr_new(&result, 4096), cu4);

    // ok, now we can actually run a test
    size_t outlen;
    size_t amnt_read;
    PROP_GO( mangle_hook(infd, temp, &outlen), cu5);
    // reset temp file descriptor for reading
    off_t oret = lseek(temp, 0, SEEK_SET);
    if(oret == (off_t) -1){
        LOG_ERROR("%x: %x\n", FS("lseek"), FE(&errno));
        ORIG_GO(E_OS, "lseek failed", cu5);
    }
    // read the test result
    while(true){
        PROP_GO( dstr_read(temp, &result, 4096, &amnt_read), cu5);
        if(!amnt_read) break;
    }
    // read the answer
    while(true){
        PROP_GO( dstr_read(outfd, &answer, 4096, &amnt_read), cu5);
        if(!amnt_read) break;
    }
    // compare result to the answer
    if(outlen != answer.len){
        ORIG_GO(E_VALUE, "wrong outlen", cu5);
    }
    if(dstr_cmp(&answer, &result) != 0){
        ORIG_GO(E_VALUE, "result does not match answer", cu5);
    }
...
static derr_t test_mangle_unencrypted(void){
    ...
    PROP( FMT(&inpath, "%x/mangle_unencrypted/in_short", FS(g_test_files)) );
    PROP( FMT(&outpath, "%x/mangle_unencrypted/out_short", FS(g_test_files)) );
    PROP( do_test_mangle(inpath.data, outpath.data, ditm_mangle_unencrypted) );

    inpath.len = 0;
    outpath.len = 0;
    PROP( FMT(&inpath, "%x/mangle_unencrypted/in_long", FS(g_test_files)) );
    PROP( FMT(&outpath, "%x/mangle_unencrypted/out_long", FS(g_test_files)) );
    PROP( do_test_mangle(inpath.data, outpath.data, ditm_mangle_unencrypted) );

    inpath.len = 0;
    outpath.len = 0;
    PROP( FMT(&inpath, "%x/mangle_unencrypted/in_nosubj", FS(g_test_files)) );
    PROP( FMT(&outpath, "%x/mangle_unencrypted/out_nosubj", FS(g_test_files)) );
    PROP( do_test_mangle(inpath.data, outpath.data, ditm_mangle_unencrypted) );
...
static derr_t test_mangle_corrupted(void){
    ...
    PROP( dstr_write_file(infile, &input) );
    PROP_GO( dstr_write_file(outfile, &output), cu1);

    // do the test
    PROP_GO( do_test_mangle(infile, outfile, ditm_mangle_corrupted), cu2);
cu2:
    remove(outfile);
cu1:
    remove(infile);
...
static derr_t test_ignore_list(void){
    ...
    if(ret != 0){
        ORIG(E_OS, "unable to make ignore directory");
    }
    // allocate/load up the test list
    ignore_list_t il;
    DSTR_VAR(path, 4096);
    PROP_GO( FMT(&path, "%x/ignore_list", FS(g_test_files)), cu1);
    PROP_GO( ignore_list_load(&il, &path), cu1);
    ...
    for(size_t i = 0; i < pos_queries.len; i++){
        ...
        if(ignore_list_should_ignore(&il, &sub) != true){
            ORIG_GO(E_VALUE, "should_ignore() incorrect result", cu2);
        }
    }
    ...
    for(size_t i = 0; i < neg_queries.len; i++){
        dstr_t sub = dstr_sub(&neg_queries, i, i+1);
        PROP_GO( ignore_list_add(&il, &sub), cu2);
    }

    // write the resulting list
    DSTR_STATIC(outdir, "test_ignore_dir");
    PROP_GO( ignore_list_write(&il, &outdir), cu2);

    // read the result into memory
    DSTR_VAR(result, 4096);
    PROP_GO( dstr_fread_file("test_ignore_dir/ignore.json", &result), cu2);

    // read the answer into memory
    path.len = 0;
    PROP_GO( FMT(&path, "%x/ignore_list/out", FS(g_test_files)), cu2);
    DSTR_VAR(answer, 4096);
    PROP_GO( dstr_fread_file(path.data, &answer), cu2);

    if(dstr_cmp(&result, &answer) != 0){
        ORIG_GO(E_VALUE, "ignore list result did not match answer", cu2);
    }
...
int main(int argc, char** argv){
    ...
    if(!dir_rw_access(ditm_path, true)){
        ORIG_GO(E_FS, "unable to create ditm_dir", test_fail);
    }
    PROP_GO( test_ditm(), test_fail);
    PROP_GO( rm_rf(ditm_path), test_fail);

    PROP_GO( test_mangle_unencrypted(), test_fail);
    PROP_GO( test_mangle_corrupted(), test_fail);
    PROP_GO( test_ignore_list(), test_fail);
