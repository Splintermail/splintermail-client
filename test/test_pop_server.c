...
static derr_t stathook(void* arg){
    ...
    last_called = called;
    called = STATHOOK;
    PROP( pop_server_send_dstr(((pop_server_t*)arg), &stat_resp) );
    return E_OK;
}
static derr_t listhook(void* arg, int index){
    ...
    called = LISTHOOK;
    g_index = index;
    PROP( pop_server_send_dstr(((pop_server_t*)arg), &list_resp) );
    return E_OK;
}
static derr_t retrhook(void* arg, unsigned int index){
    ...
    called = RETRHOOK;
    g_index = index;
    PROP( pop_server_send_dstr(((pop_server_t*)arg), &retr_resp) );
    return E_OK;
}
static derr_t delehook(void* arg, unsigned int index){
    ...
    called = DELEHOOK;
    g_index = index;
    PROP( pop_server_send_dstr(((pop_server_t*)arg), &dele_resp) );
    return E_OK;
}
static derr_t rsethook(void* arg){
    ...
    last_called = called;
    called = RSETHOOK;
    PROP( pop_server_send_dstr(((pop_server_t*)arg), &rset_resp) );
    return E_OK;
}
static derr_t tophook(void* arg, unsigned int index, unsigned int lines){
    ...
    g_index = index;
    g_lines = lines;
    PROP( pop_server_send_dstr(((pop_server_t*)arg), &top_resp) );
    return E_OK;
}
static derr_t uidlhook(void* arg, int index){
    ...
    called = UIDLHOOK;
    g_index = index;
    PROP( pop_server_send_dstr(((pop_server_t*)arg), &uidl_resp) );
    return E_OK;
}
...
static void* server_thread(void* arg){
    ...
    PROP_GO( FMT(&certfile, "%x/%x", FS(g_test_files), FS("ssl/good-cert.pem")), early_fail);
    PROP_GO( FMT(&keyfile, "%x/%x", FS(g_test_files), FS("ssl/good-key.pem")), early_fail);
    PROP_GO( FMT(&dhfile, "%x/%x", FS(g_test_files), FS("ssl/dh_4096.pem")), early_fail);
    ssl_context_t ctx;
    thread_return = ssl_context_new_server(&ctx, certfile.data,
                                                 keyfile.data,
                                                 dhfile.data);
    if(thread_return) return NULL;

    listener_t listener;
    thread_return = listener_new_ssl(&listener, &ctx, "127.0.0.1", 1996);
early_fail:
    thread_return = error;
    ...
    if(thread_return) goto cleanup;


    connection_t conn;
    // accept a connection
    thread_return = listener_accept(&listener, &conn);
    ...
    pop_server_send_dstr(&ps, &response);


    // kick off server loop
    thread_return = pop_server_loop(&ps, (void*)&ps);
...
#define EXPECT_RESPONSE(namestr, string){ \
    ...
    DSTR_VAR(recv, 4096); \
    PROP_GO( connection_read(&conn, &recv, NULL), cleanup_3); \
    /* compare to expected string */ \
    result = dstr_cmp(&string, &recv); \
    if(result != 0) \
        ORIG_GO(E_VALUE, "incorrect " namestr " response", cleanup_3); \
    }

#define EXPECT_HOOK_RESPONSE(namestr, hook, string){ \
    EXPECT_RESPONSE(namestr, string) \
    if(called != hook) \
        ORIG_GO(E_VALUE, namestr " not called", cleanup_3); \
    /* make sure it was the only hook called */ \
    if(last_called != CLEARED) \
        ORIG_GO(E_VALUE, "double hook call detected on" namestr, cleanup_3); \
    /* reset hooks */ \
    called = CLEARED; \
    last_called = CLEARED; \
}
...
static derr_t test_pop_server(void){
    ...
    ssl_context_t ctx;
    PROP_GO( ssl_context_new_client(&ctx), cleanup_1);

    // now connect to the server
    connection_t conn;
    PROP_GO( connection_new_ssl(&conn, &ctx, "127.0.0.1", 1996), cleanup_2);
    ...
    // loginhook
    {
        ...
        PROP_GO( connection_write(&conn, &username_string), cleanup_3);
        DSTR_STATIC(expect, "+OK\r\n");
        EXPECT_RESPONSE("user", expect);
        // PASS command
        DSTR_STATIC(password_string, "PASS pass word\r\n");
        PROP_GO( connection_write(&conn, &password_string), cleanup_3);
    ...
    // stathook
    {
        ...
        PROP_GO( connection_write(&conn, &temp), cleanup_3);
...
int main(int argc, char** argv){
    ...
    PROP_GO( test_pop_server(), test_fail);
