...
static derr_t cmd_event_new(cmd_event_t **out, imap_ditm_t *ditm){
    derr_t e = E_OK;
    *out = NULL;

    cmd_event_t *cmd_ev = malloc(sizeof(*cmd_ev));
    if(!cmd_ev) ORIG(&e, E_NOMEM, "nomem");
    *cmd_ev = (cmd_event_t){0};

    event_prep(&cmd_ev->ev, imap_ditm_cmd_ev_returner, ditm);
...
static void cmd_event_free(cmd_event_t **cmd_ev){
    if(!*cmd_ev) return;
    *cmd_ev = NULL;
}

// static derr_t resp_event_new(resp_event_t **out, imap_ditm_t *ditm){
//     derr_t e = E_OK;
//     *out = NULL;
//
//     resp_event_t *resp_ev = malloc(sizeof(*resp_ev));
//     if(!resp_ev) ORIG(&e, E_NOMEM, "nomem");
//     *resp_ev = (resp_event_t){0};
//
//     event_prep(&resp_ev->ev, imap_ditm_resp_ev_returner, ditm);
//     resp_ev->ev.ev_type = EV_COMMAND;
//
//     *out = resp_ev;
//     return e;
// }
//
// static void resp_event_free(resp_event_t **resp_ev){
//     if(!*resp_ev) return;
//     *resp_ev = NULL;
// }

static derr_t relay_commands(imap_ditm_t *ditm){
    derr_t e = E_OK;

    while(!link_list_isempty(&ditm->ts.unhandled_cmds)){
        // peek at first command
        link_t *link = ditm->ts.unhandled_cmds.next;
...
static derr_t relay_responses(imap_ditm_t *ditm){
    derr_t e = E_OK;

    while(!link_list_isempty(&ditm->ts.unhandled_resps)){
        // peek at first command
        link_t *link = ditm->ts.unhandled_resps.next;
...
derr_t imap_ditm_do_work(imap_ditm_t *ditm){
    ...
    PROP(&e, relay_commands(ditm) );

    PROP(&e, relay_responses(ditm) );
