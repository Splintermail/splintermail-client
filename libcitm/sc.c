...
static void sawait_cb(
    ...
    if(sc->failed){
        DROP_VAR(&e);
    }else if(sc->broken_conn || sc->canceled){
        DROP_CANCELED_VAR(&e);
    }else{
...
static void cawait_cb(
    ...
    if(sc->failed){
        DROP_VAR(&e);
    }else if(sc->broken_conn || sc->canceled || sc->logout){
        DROP_CANCELED_VAR(&e);
    }else{
        UPGRADE_CANCELED_VAR(&e, E_INTERNAL);
    }
...
static void send_cmd(derr_t *e, sc_t *sc, imap_cmd_t *cmd){
    cmd = imap_cmd_assert_writable(e, cmd, &sc->c->exts);
...
static derr_t advance_disconnect(sc_t *sc, bool expunge, bool *okout){
    derr_t e = E_OK;
...
static derr_t advance_select(
    ...
    *ok = false;
    ...
    ONCE(sc->select_connected){
        // connect our dn_t first, to pick the initial SELECT/EXAMINE correctly
        PROP(&e,
            dirmgr_open_dn(
                sc->dirmgr,
                ie_mailbox_name(*mp),
    ...
    if(!sc->select_success){
        /* first, hard-disconnect the dn_t, because dn_disconnect cannot be
           safely called after SELECT failed */
        dirmgr_close_dn(sc->dirmgr, &sc->dn);
        sc->dn_active = false;
...
static derr_t require_selected(
    ...
    /* key off of selected_mailbox, since advance_disconnected will set
       dn_active=false before the disconnection is complete, but it frees
       selected_mailbox only at the end of the disconnection */
    if(sc->selected_mailbox){
        *valid = true;
        return e;
    }
...
static derr_t send_enable(sc_t *sc){
    derr_t e = E_OK;
...
static derr_t check_enable(
    sc_t *sc, ie_dstr_t **cmdtagp, imap_resp_t **respp, bool *okout
){
    derr_t e = E_OK;
    ...
    *okout = false;
    ...
    // detect ENABLED response
    if(resp->type == IMAP_RESP_ENABLED){
        ...
        for(ie_dstr_t *ptr = resp->arg.enabled; ptr; ptr = ptr->next){
            ...
            ORIG(&e, E_RESPONSE, "bad ENABLED extension in: %x", FIRESP(resp));
        }
        if(!condstore) TRACE(&e, "missing CONDSTORE in ENABLED response\n");
        if(!qresync) TRACE(&e, "missing QRESYNC in ENABLED response\n");
        if(!condstore || !qresync){
            ORIG(&e, E_RESPONSE, "bad ENABLED response: %x", FIRESP(resp));
        }
    ...
    /* ENABLE is always the first message we send, and we don't block on the
       response, so we can't rely on sc->ntags */
    size_t enable_tag = 1;
    ie_st_resp_t *st;
    ok = (st = match_tagged(resp, prefix, enable_tag));
    if(!ok) return e;

    // make sure ENABLE succeeded
    if(st->status != IE_ST_OK){
        ORIG(&e, E_RESPONSE, "expected ENABLE to succeed");
    }
...
static derr_t advance_enable(sc_t *sc, bool *okout){
    derr_t e = E_OK;

    *okout = false;
    bool ok;

    ONCE(sc->enable_sent) PROP(&e, send_enable(sc) );

    ok = advance_reads_up(sc);
    if(!ok) return e;

    ie_dstr_t **cmdtagp = &sc->cmd->tag;
    PROP(&e, check_resp(sc, cmdtagp, &ok, check_enable) );
    if(!ok) return e;
...
static derr_t advance_logout(sc_t *sc, bool *okout){
    derr_t e = E_OK;
...
static void advance_state(sc_t *sc){
    ...
    if(sc->broken_conn || sc->canceled || sc->failed) goto cu;
    ...
    if(!sc->enable_done){
        PROP_GO(&sc->e, advance_enable(sc, &ok), fail);
        if(ok) sc->enable_done = true;
        // fallthru
    }
    ...
    if(sc->resp){
        ...
        if(consume){
            imap_resp_free(STEAL(imap_resp_t, &sc->resp));
            // start reading the next response
            (void)advance_reads_up(sc);
        }
    }

    // if the up_t is active, let it do work
    if(sc->up_active){
        PROP_GO(&sc->e, up_advance_state(&sc->up, &sc->cmds), fail);
    }
...
logout:
    ...
    up_ok = advance_writes_up(sc);
    dn_ok = advance_writes_dn(sc);
    // wait for logout logic and all writes to complete
    if(!ok || !up_ok || !dn_ok) return;
...
cu:
    ...
    imap_server_cancel(sc->s, sc->broken_conn && !sc->canceled);
    imap_client_cancel(sc->c);
