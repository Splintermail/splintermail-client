...
void imap_server_dump(imap_server_t *s, FILE *f, int indent);
void imap_client_dump(imap_client_t *c, FILE *f, int indent);
...
struct imap_server_t {
    ...
    // should we tell the client there was a broken upstream connection?
    bool broken_conn : 1;
    bool broken_conn_sent : 1;
    // or should we just shut down as fast as possible?
    bool canceled : 1;
