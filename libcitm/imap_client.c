...
static void advance_state(imap_client_t *c){
    ...
    if(c->canceled || c->failed) goto cu;

    // wait for greeting
    while(!c->greeting_done){
        PROP_GO(&c->e, advance_reads(c, &ok), fail);
        if(!ok) return;
        PROP_GO(&c->e, process_greeting(c, &ok), fail);
        if(!ok) continue;
    ...
    while(c->starttls && !c->starttls_done){
        ONCE(c->starttls_started) PROP_GO(&c->e, send_starttls(c), fail);
        PROP_GO(&c->e, advance_writes(c, &ok), fail);
        if(!ok) return;
        PROP_GO(&c->e, advance_reads(c, &ok), fail);
        if(!ok) return;
        PROP_GO(&c->e, process_starttls_resp(c, &ok), fail);
        if(!ok) continue;
        PROP_GO(&c->e, starttls(c), fail);
...
bool imap_client_free(imap_client_t **cptr){
    imap_client_t *c = *cptr;
    if(!c) return true;
    if(!link_list_isempty(&c->reads) || !link_list_isempty(&c->writes)){
        LOG_ERROR("imap_client_free with pending io\n");
        return false;
    }
    if(!c->awaited){
