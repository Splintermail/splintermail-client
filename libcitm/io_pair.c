...
static void connect_cb(void *data, citm_conn_t *conn_up){
    ...
    citm_conn_t *conn_up = io_pair->conn_up;
    io_pair_cb cb = io_pair->cb;
    void *data = io_pair->data;
    free(io_pair);
    cb(data, conn_dn, conn_up);
}
