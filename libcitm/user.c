...
typedef struct {
    ...
    bool done : 1; // if we ever ran out of sc's
    bool broken_conn : 1;
    bool canceled : 1; // if user_cancel() was called
    bool failed : 1; // if we hit an error
} user_t;
...
static derr_t check_sync(user_t *u, imap_resp_t **respp, bool *ok){
    ...
    bool mykey_deleted;
    PROP(&e, check_xkeysync(respp, u->kd, &mykey_deleted) );
    if(mykey_deleted) ORIG(&e, E_RESPONSE, "mykey deleted, crashing");
    if(!*respp) return e;
...
static void advance_state(user_t *u){
    ...
    ONCE(u->sync_sent) PROP_GO(&u->e, send_sync(u), fail);
...
cu:
    ...
    sc_t *sc;
    LINK_FOR_EACH(sc, &u->scs, sc_t, link){
        sc_cancel(sc, u->broken_conn && !u->canceled);
    }
    ...
    // free any stray resp
    imap_resp_free(STEAL(imap_resp_t, &u->resp));
...
// leaves s,c untouched on failures
